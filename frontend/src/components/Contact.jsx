...
export default function Contact() {
  ...
  // Build contact details from CMS or fallback
  const contactDetails = contactData
    ? [
        {
          icon: svg_icon('location'),
          label: 'Location',
          value: contactData.location || 'Rawalpindi, Punjab, Pakistan',
          href: contactData.location ? `https://maps.google.com/?q=${encodeURIComponent(contactData.location)}` : '#',
          isLink: true,
        },
        {
          icon: svg_icon('phone'),
          label: 'Phone',
          value: contactData.phone || '+92 340 8575834',
          href: contactData.phone ? `tel:${contactData.phone.replace(/\s/g,'')}` : '#',
          isLink: true,
        },
        {
          icon: svg_icon('email'),
          label: 'Email',
          value: contactData.email || 'mehtabakbar5656@gmail.com',
        ...
        },
        {
          icon: svg_icon('globe'),
          label: 'Portfolio',
          value: contactData.portfolio_url ? contactData.portfolio_url.replace(/^https?:\/\//, '') : 'mehtabakbar.com',
          href: contactData.portfolio_url || 'https://mehtabakbar.com',
          isLink: true,
        },
        ...(Array.isArray(contactData.social_links) ? contactData.social_links.map(link => ({
          icon: svg_icon(link.icon || 'globe'),
          label: link.label,
          value: link.url ? link.url.replace(/^https?:\/\//, '').split('/').pop() : link.url,
          href: link.url,
          isLink: true,
        })) : []),
      ]
